# surgeist-dialog Repository Guide
...
`surgeist-dialog` is an independent leaf repository. It owns its manifest,
dialog domain implementation, public front door, focused tests and docs, commits,
and published `main` candidate.
...
| Root integration MSRV, repository URL, and compatible pin | Root `Cargo.toml`, root `.gitmodules`, and root's committed gitlink for `crates/surgeist-dialog`, when root integration is in scope |
...
`surgeist-dialog` owns dialog contracts, coordination primitives, and the native
service boundary. It excludes application and window lifecycle and root
orchestration.
