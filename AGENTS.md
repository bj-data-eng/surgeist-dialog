...
Use this guide for automated work in the `surgeist-dialog` crate repo.
...
This repo owns the `surgeist-dialog` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-dialog` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-dialog` owns dialog contracts and dialog coordination primitives.
...
cargo clippy -p surgeist-dialog --all-targets -- -D warnings
cargo fmt --check
```
...
1. Confirm the work belongs in `surgeist-dialog`.
